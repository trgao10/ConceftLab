function [tfr, tfrtic, sqPsift, sqDPsift, dtfr] = CWTDW(x, opts)
%
% Continuous wavelet transform ver 0.1
% Modified from wavelab 85
%
% INPUT:
% OUTPUT: tfrtic returned as such is the scale tic (not frequency)
% DEPENDENCY:
%
% by Hau-tieng Wu 2011-06-20 (hauwu@math.princeton.edu)
% by Hau-tieng Wu 2012-12-23 (hauwu@math.princeton.edu)
% by Tingran Gao Jan 29, 2017 (trgao10@math.duke.edu)
%
...
nvoice = 32;
scale = 1; %%%% choose base scale a_0=2??
oct = 1;
...
dtfr = zeros(n, nvoice*noctave);

tfrtic = scale.*(2.^((1:(nvoice*noctave))/nvoice));

sqPsift = zeros(noctave*nvoice, length(x));
sqDPsift = zeros(noctave*nvoice, length(x));

for j = 1:length(tfrtic)
    qscale = tfrtic(j);
%     omega = xi / qscale;
    omega =  xi * qscale;
    ...
    %%% In principle one could normalize each scaled wavelts to have unit
    %%% norm, but for SST this is truly meaningless; even worse, such
    %%% normalization causes serious numerical issues.
    %         windowq = windowq * sqrt(qscale);
    %         windowq = windowq ./ sqrt(qscale);
    
    sqPsift(j,:) = windowq.'; %%% non-conjugate transpose
    %%% technically the DFT of derivative of wavelts should be
    %%% 2*pi*1j*xi.'.*sqPsift(j,:)/dt, but we omit the scaling and leave
    %%% it for the rest of sqCWTbase
    sqDPsift(j,:) = 1j*xi.'.*sqPsift(j,:);
    
    what = windowq .* xhat;
    w = ifft(what);
    tfr(:,j) = transpose(w);
    
    dtfr(:,j) = transpose(ifft(sqDPsift(j,:).'.*xhat));
end
