...
sst = zeros(fLen, tLen);
freq = linspace(params.FreqBounds(1), params.FreqBounds(2), fLen)';
...
[stftcfs,freq,ts] = spectrogram(x,winfunc,length(winfunc)-1,fLen,params.fs,'yaxis');
dstftcfs = spectrogram(x,dwinfunc,length(dwinfunc)-1,fLen,params.fs,'yaxis');
...
Tx = sstalgo(stftcfs,phasetf,params.thr,params.sqType,freq,dt);
...
%------------------------------------------------------------------------
function Tx = sstalgo(stftcfs,phasetf,gamma,freqs,dt)
