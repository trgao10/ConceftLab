...
[sstResult, instFreqTic] = fsstgao(targetSig, Fs, 'hermite',...
    'ExtendSignal', false,...
    'WindowParameters', struct('NumPts',377,'Order',3,'HalfWinSpt',10),...
...
ridgeShift = omega1^2*sqrt(1+2*omega1^2+sqrt(1+omega1^2+omega1^4))*(1/(1+omega1^2)-2/((1+omega1^2+sqrt(1+omega1^2+omega1^4))^2+omega1^2));
plot(tSamples, instFreq+ridgeShift/(2*pi), 'r', 'linewidth', 1.0);
plot(tSamples, instFreq-ridgeShift/(2*pi), 'r', 'linewidth', 1.0);
legend({'','$\omega_0+\omega_1t$','$\displaystyle\omega_0+\omega_1t\pm \omega_1^2\sqrt{1+2\omega_1^2+\sqrt{1+\omega_1^2+\omega_1^4}}\left(\frac{1}{1+\omega_1^2}-\frac{2}{\left(1+\omega_1^2+\sqrt{1+\omega_1^2+\omega_1^4}\right)^2+\omega_1^2}\right)$'}, 'Interpreter', 'latex', 'Fontsize', 11, 'Location', 'Southeast');
