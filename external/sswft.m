...
% - calculate synchrosqueezed windowed Fourier transform [SWFT] of a signal [sig] sampled at [fs] Hz.
% 
% INPUT:
...
% 'CutEdges':{{'off'}}|'on'
%           determine should SWFT coefficients be set to NaNs out of the cone
%           of influence (see [1]); set it to 'on' if you wish to analyze
%           only SWFT within the cone of influence (which is recommended if
...
% /additional to the properties of <wft.m>:/
% 'CutFreq':{{'on'}}|'off'
...
if nargin>2 && isstruct(varargin{1})
    ...
    if isfield(copt,'CutFreq'), cvv=copt.CutFreq; if ~isempty(cvv), CutFreq=cvv; end, end
    if isfield(copt,'Threshold'), cvv=copt.Threshold; if ~isempty(cvv), Threshold=cvv; end, end
    if isfield(copt,'wp')
        recflag=0;
...
for vn=vst:2:nargin-2
    ...
    elseif strcmpi(varargin{vn},'CutFreq'), if ~isempty(varargin{vn+1}), CutFreq=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'Threshold'), if ~isempty(varargin{vn+1}), Threshold=varargin{vn+1}; end
...
if ~strcmpi(PlotMode,'off')
    ...
    if isempty(strfind(lower(PlotMode),'wr')) && (size(ZZ,1)>MYL || size(ZZ,2)>MXL)
        if strcmpi(DispMode,'on')
            fprintf('Plotting: SWFT contains more data points (%d x %d) than pixels in the plot, so for a\n',size(ZZ,1),size(ZZ,2));
