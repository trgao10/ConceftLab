%================= Extract ridge curve from SWFT or SWT ===================
% Version 2.20 stable
%-------------------------------Copyright----------------------------------
%
...
%   signal. TFR can be either SWFT or SWT.
%
% OUTPUT:
% tfsupp: 3xL matrix
...
%      returned as a third output by functions sswft.m and sswt.m;
...
%      frequency - df/dt (in Hz/s) - for the SWFT, or of the log-frequency
%      - d\log(f)/dt - for the SWT; (method 2) the minimal distinguishable
%      frequency difference (in Hz) for SWFT, or log-difference for SWT.
%
% PROPERTIES: ({{...}} denotes default)
% ################################ BASIC ##################################
% 'RidgeEst':'ridge'|{{'direct'}}|'maxpeak'
%    - determines the way how to estimate ridge point amplitudes and
%      frequencies: 'direct' - from the overall region of nonzero
%      SWFT/SWT amplitude (direct reconstruction [1]); 'ridge' - from the
%      individual peaks (ridge reconstruction [1]); 'maxpeak' - from the
%      highest peaks in each region of non-zero TFR ampltiude. As
%      discussed in [3], the options 'ridge' and 'maxpeak' are generally
%      inappropriate, although can work well in particular cases. Note,
%      that there is no such property 'RidgeEst' for the ecurve.m (curve
%      extraction from usual WFT/WT), as there one can always use peak
%      values which are well defined, noise-robust and fast to find.
...
%      (on a logarithmic scale for SWT), use imaginary profile 1i*[efreq],
...
%         [] (no parameters) - for all other methods
...
%    - penalization functions used for curve extraction. For the SWFT:
...
%      For the SWT all is the same but all frequency variables are taken
...
% 4th argument, e.g. /opt.Method=1; ssecurve(TFR,freq,wopt,opt);/.
% If the other properties are specified next, they override those in the
% structure, e.g. /ssecurve(TFR,freq,wopt,opt,'Method',2);/ will
...
% - for the case when 'RidgeEst'='ridge', introduced interpolation of the
%   differences between ridge frequencies to reduce the discretization
%   errors of the corresponding median/range estimates, which might
%   otherwise be huge, considerably affecting performance
...
if nargin>3 && isstruct(varargin{1})
    ...
    if isfield(copt,'RidgeEst'), cvv=copt.RidgeEst; if ~isempty(cvv), RidgeEst=cvv; end, end %/////////////////////////////////
end
for vn=vst:2:nargin-3
    ...
    elseif strcmpi(varargin{vn},'RidgeEst'), if ~isempty(varargin{vn+1}), RidgeEst=varargin{vn+1}; end %/////////////////////////////////
...
%//////////////////////////////////////////////////////////////////////////
if strcmpi(RidgeEst,'ridge'), RidgeEst=1; elseif strcmpi(RidgeEst,'direct'), RidgeEst=2; else RidgeEst=3; end
nfunc=ones(NF,1); tn1=find(~isnan(TFR(end,:)),1,'first'); tn2=find(~isnan(TFR(end,:)),1,'last'); sflag=0;
if ischar(method) || length(method)==1 || max(abs(imag(method(:))))>0 %if not frequency-based extraction (or frequency-based with nearest selection)
    sflag=1;
    %----------------------------------------------------------------------
    ...
    if ~isempty(Skel)
        ...
        if RidgeEst==1
            RidgeEst=1; if ~strcmpi(DispMode,'off') && ~strcmpi(DispMode,'notify'), fprintf('Locating peaks in TFR... '); end
        elseif RidgeEst==2
            RidgeEst=2; if ~strcmpi(DispMode,'off') && ~strcmpi(DispMode,'notify'), fprintf('Locating supports in TFR and joining them into peaks... '); end
        else
            RidgeEst=3; if ~strcmpi(DispMode,'off') && ~strcmpi(DispMode,'notify'), fprintf('Locating supports in TFR and maximum peaks in them... '); end
        end
        ...
        if RidgeEst==1
            ...
            dind=[0;find(diff(idt(:))>0);length(idt)]; Mp=max(diff(dind));
            ...
            Ip(idnt)=idf; Fp(idnt)=freq(idf); Qp(idnt)=TFR(idft);
            clear idft idf idt idn dind idnt;
        else
            ...
            for kn=1:length(idft1)
                ...
                if RidgeEst==2
                    ...
                    [casig,idm]=max(abs(TFR(ii))); cfsig=freq(iif(idm));
                end
                %Assign all
                if idt1(kn)~=tn, nid=1; else nid=nid+1; end
                tn=idt1(kn); Fp(nid,tn)=cfsig; Qp(nid,tn)=abs(casig);
                if fres==1, fid=1+(cfsig-freq(1))/fstep;
                else fid=1+(log(cfsig)-log(freq(1)))/fstep; end
                Ip(nid,tn)=min([max([fid,1]),NF]);
            end
    ...
    if ~strcmpi(DispMode,'off') && ~strcmpi(DispMode,'notify')
        fprintf('Extracting the ridge curve lying in the same TFR supports as the specified frequency profile.\n');
    end
    ...
    for tn=tn1:tn2
        ...
        %Find ridge frequency
        ii=idown:iup;
        if RidgeEst==1
            cv=abs(cs(ii)); idp=1+find(cv(2:end-1)>=cv(1:end-2) & cv(2:end-1)>cv(3:end));
            idp=ii(idp); [~,idm]=min(abs(cind-idp)); tfsupp(1,tn)=freq(idp(idm));
            pamp(tn)=abs(cs(idp(idm)));
        elseif RidgeEst==2
            cv=TFR(ii,tn); cfsig=real(sum(freq(ii).*cv)/sum(cv));
            if cfsig>=freq(ii(1)) && cfsig<=freq(ii(end)), tfsupp(1,tn)=cfsig;
            else tfsupp(1,tn)=sum(freq(ii).*cs(ii))/sum(cs(ii)); end
            pamp(tn)=abs(sum(cv));
        else
            [pamp(tn),idm]=max(abs(cs(ii))); tfsupp(1,tn)=freq(ii(idm));
        end
    end
    %Transform to frequencies
    if fres==1, pind=1+floor(0.5+(tfsupp(1,:)-freq(1))/fstep);
    else pind=1+floor(0.5+log(tfsupp(1,:)/freq(1))/fstep); end
...
%Frequency-based extraction with nearest ridges
if ~ischar(method) && length(method)>1 && max(abs(imag(method(:))))>0
    efreq=imag(method);
    if ~strcmpi(DispMode,'off') && ~strcmpi(DispMode,'notify')
        fprintf('Extracting the ridge curve lying nearest to the specified frequency profile.\n');
    end
    
    tn1=max([tn1,find(~isnan(efreq),1,'first')]); tn2=min([tn2,find(~isnan(efreq),1,'last')]);
    if fres==1
        for tn=tn1:tn2, [~,pind(tn)]=min(abs(efreq(tn)-Fp(1:Np(tn),tn))); end
    else
        for tn=tn1:tn2, [~,pind(tn)]=min(abs(log(efreq(tn))-log(Fp(1:Np(tn),tn)))); end
    end
    lid=sub2ind(size(Fp),pind(tn1:tn2),tn1:tn2);
    tfsupp(1,tn1:tn2)=Fp(lid); pind(tn1:tn2)=round(Ip(lid)); pamp(tn1:tn2)=Qp(lid);
    
    if nargout>1, ec.pfreq=tfsupp(1,:); ec.pind=pind; ec.pamp=pamp; end
    if ~isempty(strfind(DispMode,'plot'))
        scrsz=get(0,'ScreenSize'); figure('Position',[scrsz(3)/4,scrsz(4)/8,2*scrsz(3)/3,2*scrsz(4)/3]);
        ax=axes('Position',[0.1,0.1,0.8,0.8],'FontSize',16); hold all;
        title(ax(1),'Ridge curve \omega_p(t)/2\pi'); ylabel(ax(1),'Frequency (Hz)'); xlabel(ax(1),'Time (s)');
...
if length(pars)==1
    ...
    if isempty(PenalFunc{2}), logw2=[]; else logw2=@(x)PenalFunc{2}(fs*x,DF); end
    %Main part
...
if length(pars)==2 && method==2
    ...
    if RidgeEst~=2 %suppress discretization errors by interpolation
        i1=find(abs(dpf)>0.5*fstep & abs(dpf)<1.5*fstep); i2=find(abs(dpf)>1.5*fstep);
        ii=unique([1,length(pf),i1+1,i2-1,i2,i2+1,i2+2]); ii=ii(ii>=1 & ii<=length(pf));
        pf=interp1(ii,pf(ii),1:length(pf),'pchip'); dpf=diff(pf);
    end
