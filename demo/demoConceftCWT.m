...
[CWTRep, tfrtic, sqCWTRepClean, sqCWT_tic] = sqCWTbase(clean, 0, 1/5, 2e-4, 1, h(1,:)', Dh(1,:)', Smooth, 0);
[~, ~, ~, ConceftRepClean, Conceft_tic] = ConceFT_STFT(clean, 0, 1/5, 2e-4, 1, NPts, J, HTS, MT, Smooth, Hemi);

DeltaT = tfrtic(2)-tfrtic(1);
sqSTFT_itvPS = abs((sqCWTRepClean./( 2 * DeltaT))).^2;
Conceft_itvPS = abs((ConceftRepClean./( 2 * DeltaT))).^2;
...
subplot(2,1,1);
imagesc(time(segIdx)-time(segIdx(1)-1), sqCWT_tic*Hz, log(1+abs(sqSTFT_itvPS)), [LLcwt(1) HHcwt(1)]); colormap(1-gray);
...
subplot(2,1,2);
imagesc(time(segIdx)-time(segIdx(1)-1), Conceft_tic*Hz, log(1+abs(Conceft_itvPS)), [LLcwt(2) HHcwt(2)]); colormap(1-gray);
axis xy; set(gca,'fontsize', 20); axis([0 inf 0 20]);
set(gca, 'xtick', []); ylabel('Freq (Hz)');
